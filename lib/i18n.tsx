...
export type Locale = "zh-TW" | "zh" | "en" | "vi";

const translations: Record<Exclude<Locale, "vi">, Record<string, string>> = {
  "zh-TW": {
    // layout
...
const viTranslations: Record<string, string> = {
  "site.title": "Bảng điều khiển OpenClaw Bot",
  "site.desc": "Xem cấu hình bot OpenClaw",

  "nav.menu": "Menu",
  "nav.office": "Văn phòng",
  "nav.description": "Mô tả",
  "nav.skills": "Kỹ năng",
  ...
  "home.globalTrend": "Xu hướng thống kê toàn cục",
  "home.testAll": "Kiểm tra model đã gán",
  "home.testingAll": "Đang kiểm tra model...",
  ...
  "agent.lastActive": "Hoạt động gần nhất",
  "agent.todayAvgResponse": "Phản hồi trung bình",
  "agent.status.working": "Đang chạy",
  "agent.status.online": "Trực tuyến",
  "agent.status.idle": "Rảnh",
  ...
  "pixelOffice.resetView": "Đặt lại góc nhìn",
  "pixelOffice.state.working": "Đang làm việc",
  "pixelOffice.state.idle": "Đang rảnh",
  "pixelOffice.state.offline": "Đã offline",
  "pixelOffice.state.waiting": "Đang chờ",
  "pixelOffice.tempWorker": "Worker tạm",
  "pixelOffice.gatewaySre.name": "SRE trực ca",
};
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (saved && (saved === "zh-TW" || saved === "zh" || saved === "en" || saved === "vi")) {
  ...
  const t = useCallback(
    (key: string) => {
      if (locale === "vi") {
        return viTranslations[key] ?? translations.en[key] ?? translations.zh[key] ?? key;
      }
