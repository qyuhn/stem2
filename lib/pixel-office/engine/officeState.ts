...
type OfficeLocale = 'zh-TW' | 'zh' | 'en' | 'vi'
...
export interface OfficeStateOptions {
  singlePlayerKeyboardMode?: boolean
}
...
export class OfficeState {
  ...
  private singlePlayerKeyboardMode = false
  ...
  private buildRuntimeBlockedTiles(furniture: PlacedFurniture[]): Set<string> {
    // Keep all seat tiles walkable so characters can path to and sit on
    // any chair/bench seat in the office.
  ...
  constructor(layout?: OfficeLayout, locale: OfficeLocale = 'zh', options?: OfficeStateOptions) {
    this.singlePlayerKeyboardMode = !!options?.singlePlayerKeyboardMode
    this.locale = locale
    ...
    if (!this.singlePlayerKeyboardMode) {
      this.spawnCat()
      this.spawnLobster()
    ...
    this.bugSystem = new BugSystem(this.bugWorldWidth, this.bugWorldHeight, this.singlePlayerKeyboardMode ? 0 : BUG_DEFAULT_COUNT)
  }
  ...
  walkToTile(agentId: number, col: number, row: number): boolean {
    ...
    if (targetSeatId) {
      const targetSeat = this.seats.get(targetSeatId)
      if (targetSeat && (!targetSeat.assigned || ch.seatId === targetSeatId)) {
